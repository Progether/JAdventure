...
public class Menus {
    ...
    protected MenuItem selectMenu(List<MenuItem> m) {
        ...
        String command = QueueProvider.getInstance().take();
        if (commandMap.containsKey(command.toLowerCase())) {
            return commandMap.get(command.toLowerCase());
        } else {
            QueueProvider.getInstance().offer("I don't know what '" + command + "' means.");
    ...
    private void printMenuItems(List<MenuItem> m) {
        ...
        for (MenuItem menuItem: m) {
            if (menuItem.getDescription() != null) {
                QueueProvider.getInstance().offer("[" + i + "] " + menuItem.getCommand() + " - " + menuItem.getDescription());
            } else {
                QueueProvider.getInstance().offer("[" + i + "] " + menuItem.getCommand());
            }
            i++;
        }
    }
